...
const ASRComponent: React.FC<ASRComponentProps> = ({
    ...
    const handleResult = useCallback((data: string) => {
        try {
            ...
            console.log('处理识别结果:', {
                ...
                currentTemp: tempText,
                timestamp: new Date().toISOString()
            });

            if (isFinal) {
                // 最终结果：将当前句子追加到总结果中
                setResultText(prevResult => {
                    // 科大讯飞的最终结果是完整的一句话，直接拼接
                    // 句子之间自动添加标点或空格
                    const newText = prevResult + (prevResult ? '' : '') + text;
                    console.log('拼接最终文本:', {
                        prevResult,
                ...
                console.log("更新最终文本", resultText);
            } else {
                // 中间结果：更新当前句子的临时显示
                // 注意：中间结果是当前正在说的句子的实时识别，会不断更新
                console.log('更新临时文本:', text);
                setTempText(text);
            }
        } catch (err) {
            console.error('处理识别结果失败:', err);
            // 不要因为单个结果解析失败就断开连接，只记录错误
        }
    ...
    const connectWebSocket = useCallback(() => {
        ...
        try {
            ...
            ws.onerror = (event) => {
                ...
                // 如果是连接错误且还有重连机会，尝试重连
                if (reconnectAttempts.current < maxReconnectAttempts && currentStatus !== 'CLOSING') {
    ...
    }, [localConfig, changeStatus, handleError, initRecorder, handleResult, clearTimers, status]);
    ...
    const displayText = React.useMemo(() => {
        // 使用数组 join 的方式可以优雅地处理空格，并用 trim() 去除首尾可能出现的空格
        return [resultText, tempText].filter(Boolean).join(' ');
    }, [resultText, tempText]);
